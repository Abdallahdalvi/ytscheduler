...
export function analyzeVideoSeo(input: SeoInput): SeoResult {
  ...
  if (isShort) {
    if (title.toLowerCase().includes('short') || descLower.includes('#shorts')) score += 5
    else suggestions.push('For Shorts, include "Short" or "#shorts" in title/description.')
  } else {
    score += 5
  }
